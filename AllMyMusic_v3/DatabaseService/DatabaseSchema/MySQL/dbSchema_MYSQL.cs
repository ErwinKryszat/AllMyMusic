...
namespace AllMyMusic_v3.Database
{
    public class dbSchema_MYSQL : IDisposable
    {
        ...
        public void CreateSchema()
        {
            try
            {
                con = new MySqlConnection(_dbConnectionInfo.GetConnectionString());
                con.Open();

                if (IsAdminLoggedIn() == true)
                {
                    CreateSchemaFromScript("MySQL.Tables.sql", ";");
                    CreateSchemaFromScript("MySQL.Views.sql", ";");
                    CreateSchemaFromScript("MySQL.Procedures.sql", "-- GO");
        ...
        public void UpdateSchema()
        {
            try
            {
                ...
                if (IsAdminLoggedIn() == true)
                {
                    ...
                    {
                        UpdateSchemaFromScript("MySQL.Tables_1_8_15_0.sql", "-- GO");
                        UpdateSchemaFromScript("MySQL.Views_1_8_15_0.sql", "-- GO");
                        UpdateSchemaFromScript("MySQL.Procedures_1_8_15_0.sql", "-- GO");
                        UpdateDBversion("1.8.15.0");
                    }

                    if (dbVersion.CompareTo(new Version("1.8.20.0")) < 0)
                    {
                        UpdateSchemaFromScript("MySQL.Tables_1_8_20_0.sql", "-- GO");
                        UpdateSchemaFromScript("MySQL.Views_1_8_20_0.sql", "-- GO");
                        UpdateSchemaFromScript("MySQL.Procedures_1_8_20_0.sql", "-- GO");
                        UpdateDBversion("1.8.20.0");
                    }

                    if (dbVersion.CompareTo(new Version("1.8.21.0")) < 0)
                    {
                        UpdateSchemaFromScript("MySQL.Procedures_1_8_21_0.sql", "-- GO");
                        UpdateDBversion("1.8.21.0");
                    }

                    if (dbVersion.CompareTo(new Version("1.8.23.0")) < 0)
                    {
                        //UpdateSchemaFromScript("MySQL.Tables_1_8_23_0.sql", "-- GO");
                        //UpdateSchemaFromScript("MySQL.Views_1_8_23_0.sql", "-- GO");
                        UpdateSchemaFromScript("MySQL.Procedures_1_8_23_0.sql", "-- GO");
        ...
        private void WriteParameter(String name, String value)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("AddParameter", con);
                cmd.CommandType = CommandType.StoredProcedure;

                param = cmd.Parameters.Add("var_Name", MySqlDbType.VarString, 16);
                param.Value = name;

                param = cmd.Parameters.Add("var_Value", MySqlDbType.VarString, 16);
                param.Value = value;
