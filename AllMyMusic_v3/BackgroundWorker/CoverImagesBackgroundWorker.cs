...
{
    public class CoverImagesBackgroundWorker : IBackgroundQueueJob, IDisposable
    {
        #region Fields
        ...
        #region Constructor
        public CoverImagesBackgroundWorker(CancellationToken ct) 
        {
            _ct = ct;
        }
        #endregion
        ...
        public async Task DoWork(object taskQueueItem)
        {
            ...
            ObservableCollection<AlbumItem> albumList = null;
            List<String> folderList = null;

            if (taskParams.UserData.GetType() == typeof(ObservableCollection<AlbumItem>))
            {
                albumList = (ObservableCollection<AlbumItem>)taskParams.UserData;

                folderList = new List<string>();
                for (int i = 0; i < albumList.Count; i++)
                {
                    folderList.Add(albumList[i].AlbumPath);
                }
            }

            if (taskParams.UserData.GetType() == typeof(List<String>))
            {
                folderList = (List<String>)taskParams.UserData;
            }
            ...
            progressData.ActionName = "Manage Images";
            ...
            CoverImageTools imageTools = new CoverImageTools(AppSettings.DatabaseSettings.DefaultDatabase);

            for (int i = 0; i < folderCreator.Folders.Count; i++)
            {
                imageCount += await imageTools.ManageImages(folderCreator.Folders[i]);
                ...
                progressData.FileCount = String.Format("{0}", imageCount);
